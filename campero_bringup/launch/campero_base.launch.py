...
def get_command_topic_and_type(robot_model):
    if robot_model == "mecanum":
        return "controller/cmd_omni_steering", "romea_mobile_base_msgs/OmniSteeringCommand"
    return "controller/cmd_skid_steering", "romea_mobile_base_msgs/SkidSteeringCommand"
...
def launch_setup(context, *args, **kwargs):
    ...
    base_configuration_file_path = (
        f'{get_package_share_directory("campero_description")}/config/campero_{robot_model}.yaml'
    )

    controller_manager_configuration_file_path = (
        f'{get_package_share_directory("campero_bringup")}/config/controller_manager.yaml'
    )

    base_controller_configuration_file_path = (
        f'{get_package_share_directory("campero_bringup")}/config/mobile_base_controller.yaml'
    )
    ...
    command_topic, command_type = get_command_topic_and_type(robot_model)

    cmd_mux = Node(
        package="romea_cmd_mux",
        executable="cmd_mux_node",
        name="cmd_mux",
        parameters=[{"topics_type": command_type}],
        remappings=[("~/out", command_topic)],
        output="screen",
    )
...
def generate_launch_description():
    ...
    return LaunchDescription(
        [
            common.declare_mode(),
            common.declare_robot_model(["rubber", "mecanum"]),
            common.declare_robot_namespace(),
            mobile_base.declare_base_name("base"),
            common.declare_robot_urdf_description(
                common.generate_robot_urdf_description("campero_bringup")
            ),
            common.declare_robot_ros2_control_description(
                common.generate_robot_ros2_control_description("campero_bringup")
            ),
